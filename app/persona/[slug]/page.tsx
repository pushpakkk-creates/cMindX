...
export default function PersonaPage() {
  ...
  const [page, setPage] = useState<PersonaPageDoc | null>(null);
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const ref = doc(db, "personaPages", slug);
        ...
        if (!snap.exists()) {
          ...
          setPage(snap.data() as PersonaPageDoc);
        }
      } catch (e) {
        console.error(e);
        setError("Failed to load persona page.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-neutral-950 text-neutral-50 flex items-center justify-center">
        <p className="text-sm text-neutral-400">Loading persona page…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-50">
      ...
      <footer className="border-t border-neutral-900 px-5 py-4 text-xs text-neutral-500 md:px-8">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span>Persona page generated from live behaviour.</span>
        </div>
      </footer>
    </div>
  );
